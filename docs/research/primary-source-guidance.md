...
W3C guidance for data tables requires semantic relationships through native `<table>`, `<th>`, and `<td>` markup; `scope`, `id`, or `headers` may be needed for complex tables. Visual styling alone is not sufficient because assistive technology depends on programmatic header/data relationships.
...
External implication: every chart that communicates data needs an accessible textual/data alternative. A native table is preferred for static tabular relationships. A table should not be converted to an ARIA grid unless it truly behaves as an interactive composite widget.

### 2. Keyboard and visible focus

All functionality must be operable by keyboard, and focus must remain visible. Native links, buttons, inputs, selects, details/summary, and tables reduce custom keyboard obligations. Composite widgets such as tabs or comboboxes must follow their documented keyboard patterns and expose state programmatically.
...
`ScholarlyArticle` is appropriate for scholarly article-like research pages when the page actually represents that content. Dataset identity should remain separate from article identity. Citation metadata should consistently identify creator, title, publisher, publication year, resource type, version, and persistent identifier where available.

Sources:

- Schema.org ScholarlyArticle: https://schema.org/ScholarlyArticle
- DataCite mandatory metadata properties: https://support.datacite.org/docs/datacite-metadata-schema-v44-mandatory-properties
- DataCite citation guidance: https://support.datacite.org/docs/datacite-citation

### 7. Generated-page SEO
